...
interface IOracle {
    function latestRoundData() external returns (uint256, int256, uint256, uint256, uint256);
}


/**
 * @title BaseToken Monetary Supply Policy
 * @dev This is an implementation of the BaseToken Ideal Money protocol.
 ...
 *      market oracles.
 */
contract BaseTokenPolicy is Ownable {
    ...
    function rebase() external onlyOrchestrator {
        ...
        uint256 mcap;
        int256 mcap_;
        (,mcap_,,,) = mcapOracle.latestRoundData();
        mcap = uint256(mcap_);
        ...
        (,exchangeRate_,,,) = marketOracle.latestRoundData();
        exchangeRate = uint256(exchangeRate_);
        ...
        emit LogRebase(epoch, exchangeRate, mcap, supplyDelta, now);
    }
